...
public class UIPCC {
	public static void main(String[] args) throws Exception {
		...
		if (debugMode) {
			configPara = new String[]{
					"./datasets/rtMatrix.txt",
					"./datasets/generatedData/RT/density_0.20/",
	...
	public static void runPredictMethods(String[] para) throws Exception {
		...
		double[][] originalMatrix = MatrixFileUtils.readMatrix(dataMatrix, userNumber, itemNumber);
		double[][] ss = MatrixFileUtils.removeEntry(originalMatrix, density);
		MatrixFileUtils.writeMatrix(ss, dataFolder+"round_01_train.txt");
		double[][]sstest = MatrixFileUtils.removeEntry(originalMatrix, density);
		MatrixFileUtils.writeMatrix(sstest, dataFolder+"round_01_test.txt");
		...
		for (int k = 0; k < ITERATIONS; k++) {
			...
			double[][] indexMatrix = MatrixFileUtils.readMatrix(traindata, userNumber, itemNumber);			
		...
		if (saveTimeInfo) {
			...
			for (int i = 0; i < timeResults.length; i++) {
				avgTimeResults[i] = MatrixFileUtils.getMeanAndStd(timeResults[i]);
			}
	...
	public static void saveEvalResult(double[][][] resultMatrix, String outFile) {
		...
		for (int i = 0; i < resultMatrix.length; i++) {
			for (int j = 0; j < resultMatrix[0].length; j++) {
				double[] stat = MatrixFileUtils.getMeanAndStd(resultMatrix[i][j]);
		...
		MatrixFileUtils.writeFile(outFile, line + "\r\n");
    }
	...
	public static double[][] predict(double[][] originalMatrix, 
		...
		double[][] originalMatrixT = MatrixFileUtils.matrixTransfer(originalMatrix);
		double[][] removedMatrixT = MatrixFileUtils.matrixTransfer(removedMatrix);
		...
		double[][] testIndexMatrix = MatrixFileUtils.readMatrix(testdata, 
		...
		double[][] testMatrixT = MatrixFileUtils.matrixTransfer(testMatrix);
