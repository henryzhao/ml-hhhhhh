package com.zyq.ml.algorithm.arima;
...
//		}
		...
		double []tempData=new double[originalData.length-300];
		for(int i=0;i<originalData.length-300;i++)
		{
			tempData[i]=originalData[i+300]-originalData[i];
		}
 ...
 */ 
	...
	{
		//seasonal Difference:Peroid=7
		double []tempData=new double[originalData.length-24];
		for(int i=0;i<originalData.length-24;i++)
		{
			tempData[i]=originalData[i+24]-originalData[i];
		}
		return tempData;
	}
